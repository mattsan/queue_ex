defmodule Queue do
  ...
  def size(queue) do
    Agent.get(queue, fn %{input: input, output: output} ->
      Enum.count(input) + Enum.count(output)
    end)
  end

  def enqueue(queue, value) do
    Agent.update(queue, fn %{input: input, output: output} ->
      %Queue{input: [value | input], output: output}
    end)
  end
